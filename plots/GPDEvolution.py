...
ax1.text(0.0012, 0.28, r"\textbf{MMHT2014lo68cl}", fontsize = 16)

ax1.set_ylabel(r"$xH_u^{(-)}(x,\xi,\mu)$", fontsize = 18)
...
ax1.plot(data[:,0], data[:,4], label = r"\textbf{$\xi=0.05$}", color = "orange")
ax1.plot(data[:,0], data[:,5], label = r"\textbf{$\xi=0.2$}",  color = "green")
ax1.plot(data[:,0], data[:,6], label = r"\textbf{$\xi=0.5$}",  color = "magenta")
ax1.plot(data[:,0], data[:,7], label = r"\textbf{$\xi=0.9$}",  color = "cyan")
ax1.plot(data[:,0], data[:,8], label = r"\textbf{$\xi=1$}",    color = "brown")
...
ax2.plot(data[:,0], data[:,4] / data[:,1], color = "orange")
ax2.plot(data[:,0], data[:,5] / data[:,1], color = "green")
ax2.plot(data[:,0], data[:,6] / data[:,1], color = "magenta")
ax2.plot(data[:,0], data[:,7] / data[:,1], color = "cyan")
ax2.plot(data[:,0], data[:,8] / data[:,1], color = "brown")
...
plt.ylabel(r"$\displaystyle \int_0^1 dx\,x^n H_u^{(-)}(x,\xi,\mu)$", fontsize = 18)
plt.xlabel(r"$\xi$")
plt.xlim([-0.1, 1.1])
plt.ylim([0.04, 10])
plt.yscale("log")
plt.plot(data[:,0], data[:,1], "ro", label = r"\textbf{$n=0$}")
plt.plot(data[:,0], data[:,2], "bo", label = r"\textbf{$n=1$}")
plt.plot(data[:,0], data[:,3], "go", label = r"\textbf{$n=2$}")
...
plt.plot(xi, p0(xi), 'r--', lw = 1.5, label = r"\textbf{Constant fit}")
plt.plot(xi, p1(xi), 'b--', lw = 1.5, label = r"\textbf{Quadratic fit}")
plt.plot(xi, p2(xi), 'g--', lw = 1.5, label = r"\textbf{Quadratic fit}")
