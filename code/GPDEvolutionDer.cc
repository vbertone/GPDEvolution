...
int main()
{
  ...
  // Construct DGLAP object
  const auto EvolGpds = BuildDglap(InitializeGpdObjects(g, Thresholds, xi), InPDFs, mu0, pto, as);

  // Tabulate GPDs
  const apfel::TabulateObject<apfel::Set<apfel::Distribution>> TabulatedGPDs{*EvolGpds, 50, 1, 1000, 3};
  ...
  const double mumax = 1000;
  const double mustp = exp( log( mumax / mumin ) / ( nmu - 1 ) );

  for (double mu = mumin; mu <= mumax * 1.0000001; mu *= mustp)
    {
      // Get derivative
      const std::map<int, apfel::Distribution> der = QCDEvToPhys(EvolGpds->Derivative(Thresholds.size(), 2 * log(mu), TabulatedGPDs.Evaluate(mu)).GetObjects());
      for (double x = xmin; x <= xmax * 1.0000001; x += xstp)
	std::cout << std::scientific << mu << "\t" << x << "\t" << der.at(2).Evaluate(x) << std::endl;
    }
  return 0;
}
